...
function Approve_Benefier(props) {
    ...
    const approve_ben = (data) => {
        console.log(data)
        props.Approve_Benefiers(data)

        setTimeout(() => {
            toast.success("Successfully Approve Benefiers", {
    ...
    const accounttransaction = () => {
        ...
        if (user != undefined) {
            ...
            for (var i = 0; i < user.length; i++) {
                for (var j = 0; j < user[i]["UsersBenficiary"].length; j++) {
                    // console.log(user[i]["UsersBenficiary"][j])
                    if (user[i]["UsersBenficiary"][j]["status"] == false) {
                        appbeneficiary.push(user[i]["UsersBenficiary"][j])
                ...
                <Row>
                    <Col lg={12} className='text-center'>
                        ...
                        <Link to="/Approve_Benefiers">
                            <Button variant="contained" style={{ marginLeft: "10px", width: "22%", margin: "1%", backgroundColor: "#01065D" }} >Approve Beneficiary</Button>
                        </Link>
                ...
                <Row>
                    <Col lg={12}>
                        ...
                        <Table bordered hover size="lg" style={{ margin: "20px", width: "90%", textAlign: "center" }}>
                            <thead>
                                <tr>
                                    <th colSpan={6} style={{ fontSize: "28px" }}>Newly Added Benefiers</th>
                                </tr>
                                <tr>
                                    <th>S.NO</th>
                                    <th>Customer Name</th>
...
const mapDispatchToProps = (dispatch) => ({
    Approve_Benefiers: (data) => dispatch(Approve_Benefiers(data)),


})


export default connect(mapStateToProps, mapDispatchToProps)(Approve_Benefier);